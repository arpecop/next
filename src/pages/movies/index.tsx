				...
				{movies.map(({ slug, title, description, id }) => (
					...
					>
							...
							<figure className='relative flex w-1/2  overflow-hidden rounded-l-lg'>
								<div className='absolute inset-0  h-full'>
									<Link href={`/movies/${id.replace('-mov', '')}`}>
										<img src={`https://klounda-s3.s3.amazonaws.com/public/filmi/${id.replace('-mov', '')}.jpg`} alt={title} />
									</Link>
								</div>
							</figure>
						...
						<div className='card-actions -mt-14 mr-2 justify-end'>
							<Link href={`/movies/m/${id}`} passHref className='btn-primary btn rounded-l-none rounded-t-none dark:btn-ghost'>
								още
							</Link>
						</div>
					</article>
