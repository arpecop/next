...
const Pagination: FC<Props> = ({ pagenum, cat, nextToken }): ReactElement => {
	...
	const lengths = ['', 'text-lg pt-2 ', 'text-md pt-2 ', 'pt-2  text-xs', 'pt-2  text-xs rotate-90']
	...
	return (
		<div className='fixed bottom-0 left-0 z-20 flex w-full justify-center bg-black/30 p-4 backdrop-blur-sm rounded-t-xl'>
			<div className='btn-group'>
				...
				<button className={className}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10 absolute">
					<path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
				</svg>
					<div className={lengths[pagenum.toString().length]}>  {pagenum}</div></button>
				{nextToken && (
					<Link
						passHref
						className={className}
